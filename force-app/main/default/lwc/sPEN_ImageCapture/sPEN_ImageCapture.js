...
import {
  ...
  ToastTypes,
  dataURLtoFile
} from "c/sPEN_UtilsImageCapture";
import WORKSTEP_NAME from "@salesforce/schema/WorkStep.Name";
import RELATEDRECORD_ID from "@salesforce/schema/WorkStep.SPEN_RelatedRecordID__c";
import WORKORDERLINEITEM_ID from "@salesforce/schema/WorkStep.WorkOrderLineItemId";

export default class SPEN_ImageCapture extends LightningElement {
  ...
  // context, and receive the record's ID when it runs
    ...
    @api
    actionMessage = "Add to record";

    @api
    addFileMessage = "Upload any supporting photos (optional)";
    ...
    get workStepFields() {
        return [WORKORDERLINEITEM_ID, WORKSTEP_NAME, RELATEDRECORD_ID];
    }
    workstep;
    ...
    get workAssetId() {
        return this.workstep?.data?.fields?.SPEN_RelatedRecordID__c?.value ?? "";
    }

    get workstepName() {
        return this.workstep?.data?.fields?.Name?.value ?? "";
    }
    ...
    get toastMessage() {
        switch (this.toastType) {
            case ToastTypes.Success: {
                ...
                if(this.gs6){
                    return `${this.numPhotosToUpload} ${imageString} added to the GS6.`;
                }else{
    ...
    for (const item of allImagesCopy) {
        ...
        const relatedRecordId = this.workAssetId.length > 0 ? this.workAssetId : this.woliId; 
    ...
    if(this.gs6){
        fullFileName = fullFileName.replaceAll("TEMP", "GS6");
    }
    ...
    this.dispatchEvent(new CustomEvent('getcontentdocumentid', {
        detail: {
            message: contentDocumentId
        }
        }));
